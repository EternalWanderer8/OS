...
{
public:
    static CStateMachine determineStateMachine(const CStateMachine& machine, CGrammarParser::GrammarSide grammarSide)
    {
        ...
        else
        {
            ...
            int startStateIndex = findStartState(machine, grammarSide);
            firstOriginalState.states.insert(states[startStateIndex]);
            if (signals[startStateIndex] == FINAL_SIGNAL)
            {
...
private:
    static inline std::string DEFAULT_START_STATE = "S";
    ...
    static int findStartState(const CStateMachine& machine, CGrammarParser::GrammarSide grammarSide)
    {
        std::vector<std::string> states = machine.getStates();

        std::string startState = grammarSide == CGrammarParser::GrammarSide::LEFT
            ? CGrammarParser::EDGE_STATE
            : DEFAULT_START_STATE;

        return std::find(states.begin(), states.end(), startState) - states.begin();
    }
};
